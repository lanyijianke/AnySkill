...
2. 再检查 `.anyskill.json` 中的 `token` 字段（其他 IDE 场景）
...
### 如果 `.anyskill.json` 已存在（老用户）

说明用户之前已经初始化过。**跳过全部初始化流程**，直接验证配置是否有效：
1. 读取 `.anyskill.json` 中的 `repo` 和 `localPath`。
2. 验证 `localPath` 目录是否存在。
3. 尝试拉取 `index.json` 确认连接正常。
4. 告知用户：
   > ✅ 已检测到 AnySkill 配置，连接到仓库 `{repo}`。随时可以使用！

如果验证失败（目录不存在或网络异常），提示用户是否需要重新初始化。
...
2. **安全存储 Token**（根据 IDE 环境区分）：
   - **OpenClaw**：将 Token 写入 `~/.openclaw/.env` 文件（追加一行 `ANYSKILL_GITHUB_TOKEN=ghp_xxx`），**不要** 将 token 写入 `.anyskill.json`。
   - **其他 IDE（Gemini/Claude Code/Cursor）**：将 token 直接写入 `.anyskill.json`。
3. 执行 clone：
```bash
...
5. **安全存储 Token**（同路径 A 的规则）：
   - **OpenClaw**：追加 `ANYSKILL_GITHUB_TOKEN=ghp_xxx` 到 `~/.openclaw/.env`。
   - **其他 IDE**：token 写入 `.anyskill.json`。
6. 在项目根目录创建 `.anyskill.json`（OpenClaw 不含 token，其他 IDE 含 token，格式同路径 A）。
...
1. 告知用户：
   ...
   > 3. 按提示输入仓库名称确认删除
   >
   > 删除后，请手动删除本地的 `.anyskill.json` 配置文件和 `{localPath}` 目录。
