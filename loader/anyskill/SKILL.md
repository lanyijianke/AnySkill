...
2. 获取其中的 `skills` 数组。
3. **逐个加载**，按照**模式一（按需加载）**或**模式二（按需下载）**的流程处理每个技能。
4. **容错机制**：如果某个技能加载失败（网络错误、索引中不存在等），**跳过并继续**加载下一个，不要中断整个流程。
5. 完成后给出**汇总报告**：
   > ✅ 组合包 `{name}` 安装完毕！
   > - 成功：{N} 个（skill-a, skill-b, skill-c）
   > - 失败：{M} 个（skill-x — 原因：索引中未找到）
   >
   > 如需重试失败的技能，请说"加载 skill-x"。
